"""
tests/test_multiple_choice_condition_flow.py — Regression tests for the
"multiple choice" condition (MultipleChoiceCondition), driven through the REAL
registered handlers (bot.py + handlers/whisper.py + handlers/conditional_whisper.py).

Covers:
  1. Correct choice unlocks the whisper and records the reader.
  2. Wrong choice keeps the cond_answer state and records a failed attempt.
  3. Retry: after a wrong choice, the correct choice still unlocks.
  4. Opening the whisper after a correct choice delivers the content.
  5. The reader is recorded exactly once.
  6. The creation wizard stores question/choices/correct_index inside conditions_data.

Flow under test:
  create conditional whisper -> /start view_<wid> -> read:<wid> callback
  -> question + choice buttons (state=cond_answer) -> wrong choice keeps state
  -> correct choice -> content delivered + reader recorded.
"""
import json
import os
import sys
import unittest
import tempfile
import atexit
from unittest.mock import MagicMock

_tmpdb = tempfile.mktemp(suffix="_mc_cond.db")
os.environ["DATABASE_PATH"] = _tmpdb
os.environ["BOT_TOKEN"]     = "0:test_token_placeholder"
os.environ["ADMIN_IDS"]     = "999"
atexit.register(lambda: os.path.exists(_tmpdb) and os.unlink(_tmpdb))

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import database as db
from database.whisper_conditions import init_whisper_conditions_db, get_condition_attempts

import bot as botmod
bot = botmod.bot
user_states = botmod.user_states

SENDER_ID = 20001
READER_ID = 20002


def _dispatch_message(msg):
    from telebot import ContinueHandling
    for handler in bot.message_handlers:
        if bot._test_message_handler(handler, msg):
            result = handler['function'](msg)
            if not isinstance(result, ContinueHandling):
                return handler['function']
    return None


def _dispatch_callback(call):
    for handler in bot.callback_query_handlers:
        if bot._test_message_handler(handler, call):
            handler['function'](call)
            return handler['function']
    return None


def _make_user(user_id, username, first_name):
    u = MagicMock()
    u.id = user_id
    u.username = username
    u.first_name = first_name
    u.last_name = None
    return u


def _make_message(user_id, text):
    msg = MagicMock()
    msg.content_type = "text"
    msg.text = text
    msg.from_user = _make_user(user_id, f"user{user_id}", f"User{user_id}")
    msg.chat = MagicMock()
    msg.chat.id = user_id
    msg.chat.type = "private"
    msg.message_id = 3000 + user_id
    msg.reply_to_message = None
    return msg


def _make_callback(user_id, data):
    call = MagicMock()
    call.id = f"cb_{data}"
    call.data = data
    call.from_user = _make_user(user_id, f"user{user_id}", f"User{user_id}")
    call.message = MagicMock()
    call.message.chat.id = user_id
    call.message.message_id = 4000 + user_id
    call.inline_message_id = None
    return call


def _mc_condition(question="What is the capital of France?", choices=("Paris", "Rome", "Madrid", "Berlin"), correct_index=0):
    return {
        "question": question,
        "choices": list(choices),
        "correct_index": correct_index,
    }


class TestMultipleChoiceConditionReadFlow(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        db.init_db()
        init_whisper_conditions_db()
        bot.send_message = MagicMock(return_value=MagicMock(message_id=1, chat=None))
        bot.answer_callback_query = MagicMock()
        bot.reply_to = MagicMock(return_value=MagicMock(message_id=2, chat=None))
        bot.edit_message_reply_markup = MagicMock()
        bot.delete_message = MagicMock()
        bot.get_me = MagicMock(return_value=MagicMock(username="testbot"))
        bot.get_chat_member = MagicMock(return_value=MagicMock(status="member"))
        if not any(getattr(h['function'], '__name__', '') == 'cwhisper_start'
                   for h in bot.callback_query_handlers):
            botmod.register_all_handlers()

    def setUp(self):
        user_states.clear()
        bot.send_message.reset_mock()
        bot.answer_callback_query.reset_mock()
        bot.reply_to.reset_mock()
        bot.edit_message_reply_markup.reset_mock()
        db.upsert_user(SENDER_ID, "sender", "Sender", None)
        db.upsert_user(READER_ID, "reader", "Reader", None)

    def _create_mc_whisper(self, content="MC SECRET"):
        return db.create_whisper(
            sender_id=SENDER_ID,
            content=content,
            whisper_type="everyone",
            conditions_data={"multiple_choice": _mc_condition()},
        )

    def _start_view(self, wid):
        msg = _make_message(READER_ID, f"/start view_{wid}")
        _dispatch_message(msg)
        return msg

    def _read_callback(self, wid):
        call = _make_callback(READER_ID, f"read:{wid}")
        _dispatch_callback(call)
        return call

    def _mc_pick(self, wid, index):
        call = _make_callback(READER_ID, f"mc_pick:{wid}:{index}")
        _dispatch_callback(call)
        return call

    def _texts_sent(self):
        return [c.args[1] for c in bot.send_message.call_args_list
                if len(c.args) >= 2 and isinstance(c.args[1], str)]

    def _alerts_shown(self):
        return [c.args[1] for c in bot.answer_callback_query.call_args_list
                if len(c.args) >= 2 and isinstance(c.args[1], str)]

    def test_registry_has_multiple_choice(self):
        from conditions import registry
        self.assertIn("multiple_choice", set(registry.all().keys()))

    def test_question_and_choices_rendered_as_buttons(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        self.assertEqual(user_states.get(READER_ID),
                         {"action": "cond_answer", "whisper_id": wid,
                          "condition_type": "multiple_choice"})
        self.assertTrue(any("What is the capital of France?" in t for t in self._texts_sent()),
                        "the question must be shown to the reader")

        button_labels = []
        for c in bot.send_message.call_args_list:
            kb = (c.kwargs or {}).get("reply_markup")
            if kb and hasattr(kb, "keyboard"):
                for row in kb.keyboard:
                    for btn in row:
                        if getattr(btn, "text", None):
                            button_labels.append(btn.text)
        for choice in ("Paris", "Rome", "Madrid", "Berlin"):
            self.assertTrue(any(choice in label for label in button_labels),
                            f"choice button '{choice}' must be rendered")

    def test_correct_choice_unlocks_and_records_reader(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        self.assertEqual(user_states.get(READER_ID).get("action"), "cond_answer")

        self._mc_pick(wid, 0)
        self.assertIsNone(user_states.get(READER_ID), "state must be popped")
        from database import get_readers
        self.assertIn(READER_ID, [r["user_id"] for r in get_readers(wid)],
                      "reader must be recorded")
        self.assertTrue(any("MC SECRET" in t for t in self._alerts_shown()),
                        "whisper content must be delivered to the reader")

    def test_wrong_choice_keeps_state_and_records_attempt(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        self.assertEqual(user_states.get(READER_ID, {}).get("action"), "cond_answer")

        self._mc_pick(wid, 1)
        self.assertEqual(user_states.get(READER_ID, {}).get("action"), "cond_answer",
                         "state must persist after a wrong choice")
        self.assertTrue(any("❌ إجابة غير صحيحة، حاول مرة أخرى." in t for t in self._alerts_shown()),
                        "wrong-choice message must be shown")
        from database import get_readers, reader_count
        self.assertEqual(reader_count(wid), 0, "no read on wrong choice")
        self.assertEqual(get_readers(wid), [])
        attempts = get_condition_attempts(wid, READER_ID)
        self.assertTrue(any(not a["passed"] for a in attempts),
                        "failed attempt must be recorded")

        self._mc_pick(wid, 0)
        self.assertIsNone(user_states.get(READER_ID))
        self.assertEqual(reader_count(wid), 1)
        self.assertTrue(any("MC SECRET" in t for t in self._alerts_shown()))

    def test_retry_after_multiple_wrong_choices(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        for _ in range(2):
            self._mc_pick(wid, 2)
            self.assertEqual(user_states.get(READER_ID, {}).get("action"), "cond_answer")

        self._mc_pick(wid, 0)
        self.assertIsNone(user_states.get(READER_ID), "retry must unlock after correct choice")
        from database import reader_count
        self.assertEqual(reader_count(wid), 1)

    def test_reader_recorded_once(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        self._mc_pick(wid, 0)
        self.assertIsNone(user_states.get(READER_ID))
        from database import get_readers, reader_count
        self.assertEqual(reader_count(wid), 1)

        self._start_view(wid)
        self._read_callback(wid)
        self._mc_pick(wid, 0)
        self.assertEqual(reader_count(wid), 1, "reader must be recorded only once")
        readers = [r["user_id"] for r in get_readers(wid)]
        self.assertEqual(readers.count(READER_ID), 1)

    def test_max_attempts_blocks(self):
        wid = self._create_mc_whisper()
        self._start_view(wid)
        self._read_callback(wid)
        for _ in range(3):
            self._mc_pick(wid, 3)
        self.assertIsNone(user_states.get(READER_ID),
                          "state must be cleared after exhausting attempts")
        from database import reader_count
        self.assertEqual(reader_count(wid), 0)


class TestMultipleChoiceConditionWizard(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        db.init_db()
        init_whisper_conditions_db()
        from database.envelope import init_envelope_db
        init_envelope_db()
        bot.send_message = MagicMock(return_value=MagicMock(message_id=1, chat=None))
        bot.answer_callback_query = MagicMock()
        bot.reply_to = MagicMock(return_value=MagicMock(message_id=2, chat=None))
        bot.edit_message_reply_markup = MagicMock()
        bot.delete_message = MagicMock()
        bot.get_me = MagicMock(return_value=MagicMock(username="testbot"))
        bot.get_chat_member = MagicMock(return_value=MagicMock(status="member"))
        if not any(getattr(h['function'], '__name__', '') == 'cwhisper_start'
                   for h in bot.callback_query_handlers):
            botmod.register_all_handlers()

    def setUp(self):
        user_states.clear()
        bot.send_message.reset_mock()
        bot.answer_callback_query.reset_mock()
        bot.reply_to.reset_mock()
        db.upsert_user(SENDER_ID, "sender", "Sender", None)

    def _texts_sent(self):
        return [c.args[1] for c in bot.send_message.call_args_list
                if len(c.args) >= 2 and isinstance(c.args[1], str)]

    def _button_labels_sent(self):
        labels = []
        for c in bot.send_message.call_args_list:
            kb = (c.kwargs or {}).get("reply_markup")
            if kb and hasattr(kb, "keyboard"):
                for row in kb.keyboard:
                    for btn in row:
                        if getattr(btn, "text", None):
                            labels.append(btn.text)
        return labels

    def test_wizard_stores_multiple_choice_config(self):
        _dispatch_callback(_make_callback(SENDER_ID, "cwhisper_start"))
        self.assertEqual(user_states.get(SENDER_ID), {"action": "cw_awaiting_condition_type"})
        self.assertTrue(any("اختيار من متعدد" in label for label in self._button_labels_sent()),
                        "multiple-choice option must be shown in the type-selection menu")

        _dispatch_callback(_make_callback(SENDER_ID, "cw_cond:multiple_choice"))
        self.assertEqual(user_states.get(SENDER_ID), {"action": "cw_awaiting_mc_question"})
        self.assertTrue(any("أرسل السؤال" in t for t in self._texts_sent()))

        _dispatch_message(_make_message(SENDER_ID, "ما عاصمة فرنسا؟"))
        self.assertEqual(user_states.get(SENDER_ID).get("action"), "cw_awaiting_mc_choice")
        self.assertTrue(any("أرسل الخيار الأول" in t for t in self._texts_sent()))

        for i in range(4):
            _dispatch_message(_make_message(SENDER_ID, f"خيار {i + 1}"))

        state = user_states.get(SENDER_ID)
        self.assertEqual(state.get("action"), "cw_awaiting_mc_correct")
        self.assertEqual(len(state.get("mc_choices")), 4)

        _dispatch_message(_make_message(SENDER_ID, "3"))
        state = user_states.get(SENDER_ID)
        self.assertEqual(state["action"], "cw_awaiting_content")
        mc = state["conditions_data"]["multiple_choice"]
        self.assertEqual(mc["question"], "ما عاصمة فرنسا؟")
        self.assertEqual(mc["choices"], ["خيار 1", "خيار 2", "خيار 3", "خيار 4"])
        self.assertEqual(mc["correct_index"], 2)

        _dispatch_message(_make_message(SENDER_ID, "همسة سرية باختيار من متعدد"))
        self.assertIsNone(user_states.get(SENDER_ID), "state must be cleared after save")
        from database.envelope import get_draft
        draft = get_draft(SENDER_ID)
        self.assertIsNotNone(draft)
        self.assertEqual(draft["content"], "همسة سرية باختيار من متعدد")
        conds = json.loads(draft["conditions_data"])
        self.assertIn("multiple_choice", conds)
        self.assertEqual(conds["multiple_choice"]["choices"],
                         ["خيار 1", "خيار 2", "خيار 3", "خيار 4"])
        self.assertEqual(conds["multiple_choice"]["correct_index"], 2)

    def test_wizard_rejects_out_of_range_correct_index(self):
        _dispatch_callback(_make_callback(SENDER_ID, "cwhisper_start"))
        _dispatch_callback(_make_callback(SENDER_ID, "cw_cond:multiple_choice"))
        _dispatch_message(_make_message(SENDER_ID, "سؤال؟"))
        for i in range(4):
            _dispatch_message(_make_message(SENDER_ID, f"خيار {i + 1}"))

        _dispatch_message(_make_message(SENDER_ID, "9"))
        self.assertEqual(user_states.get(SENDER_ID).get("action"), "cw_awaiting_mc_correct",
                         "out-of-range index must keep the state")

        _dispatch_message(_make_message(SENDER_ID, "2"))
        self.assertEqual(user_states.get(SENDER_ID).get("action"), "cw_awaiting_content")


if __name__ == "__main__":
    unittest.main(verbosity=2)
