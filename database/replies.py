...
def get_first_reader(whisper_id: str):
    ...
    Returns None if no one has read it yet.
    """
    with get_conn() as conn:
        row = conn.execute(
            "SELECT user_id FROM whisper_readers"
            " WHERE whisper_id=? ORDER BY id ASC LIMIT 1",
            (whisper_id,),
        ).fetchone()
    return row["user_id"] if row else None


def get_conversation_partner(whisper_id: str, user_id: int):
    """
    Return the user_id of the conversation partner for a given whisper participant.
    The partner is the other person in this two-way conversation:
    - If user is the sender, partner is the first reader (may be None if unread).
    - If user is the first reader, partner is the sender.
    ...
    if user_id == w["sender_id"]:
        return get_first_reader(whisper_id)

    first_reader_id = get_first_reader(whisper_id)
    if user_id == first_reader_id:
        return w["sender_id"]
...
def can_reply_to_whisper(whisper_id: str, user_id: int):
    ...
    with get_conn() as conn:
        w = conn.execute(
            "SELECT sender_id, is_locked FROM whispers WHERE whisper_id=?",
            (whisper_id,),
        ).fetchone()
