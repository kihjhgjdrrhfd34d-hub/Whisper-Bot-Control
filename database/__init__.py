...
def get_reader_ordinal(whisper_id: str, user_id: int) -> int | None:
    ...
    serializes writes (BEGIN IMMEDIATE + conditional insert), ``id`` order
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id FROM whisper_readers WHERE whisper_id=? AND user_id=?",
            (whisper_id, user_id),
        ).fetchone()
        if row is None:
            return None
        return conn.execute(
            "SELECT COUNT(*) FROM whisper_readers WHERE whisper_id=? AND id < ?",
            (whisper_id, row["id"]),
        ).fetchone()[0]
    ...
    if wtype == "custom":
        ...
        def _matches_target():
            ...
            if not username_targets:
                return False
            with get_conn() as conn:
                placeholders = ",".join("?" * len(username_targets))
                rows = conn.execute(
...
def delete_expired_whispers():
    ...
    with get_conn() as conn:
        ...
        for row in rows:
            ...
            try:
                conn.execute("DELETE FROM whisper_conditions WHERE whisper_id=?", (wid,))
            except Exception:
                pass
            try:
