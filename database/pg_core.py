...
def delete_whisper(whisper_id):
    with get_conn() as conn:
        ...
        try:
            conn.execute("DELETE FROM contact_reviews WHERE whisper_id=%s", (whisper_id,))
        except Exception:
            pass
...
def count_user_reads(user_id):
    ...
    with get_conn() as conn:
        return conn.execute(
            "SELECT COUNT(*) FROM whisper_readers WHERE user_id=%s",
            (user_id,),
        ).fetchone()["count"]
...
def delete_expired_whispers():
    ...
    with get_conn() as conn:
        ...
        for row in rows:
            ...
            try:
                conn.execute("DELETE FROM contact_reviews WHERE whisper_id=%s", (wid,))
            except Exception:
                pass
