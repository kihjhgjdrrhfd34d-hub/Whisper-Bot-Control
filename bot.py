...
from handlers.whisper import (
    register_whisper_handlers,
    handle_condition_answer_message,
    handle_whisper_start_deep_link,
)
...
def start_cmd(msg: telebot.types.Message):
    ...
    # ── Deep link: open whisper directly (e.g. /start whisper_<id>) ────
    #    Generated by the read-button start gate for users who never used
    #    the bot. The whisper opens in private through the shared read flow
    #    (handle_whisper_start_deep_link) with all access rules enforced.
    if payload and payload.startswith("whisper_"):
        handle_whisper_start_deep_link(
            bot, msg, user_states, payload[len("whisper_"):]
        )
        return
