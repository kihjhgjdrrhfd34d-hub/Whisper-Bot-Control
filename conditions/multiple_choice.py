...
from conditions.password import _filter_attempts

logger = logging.getLogger(__name__)


class MultipleChoiceCondition(BaseCondition):
    name = "multiple_choice"
    description = "Requires picking the correct option to read the whisper"
    MAX_ATTEMPTS = 3
    ...
    def _check(self, whisper_id: str, user_id: int, config: dict) -> ConditionResult:
        condition_id = config.get("id", "multiple_choice")
        ...
        choices_text = "\n".join(
            f"{i + 1}️⃣ {c}" for i, c in enumerate(choices)
        )
        message = f"🎯 {question}\n\n{choices_text}" if question else "🎯 اختر الإجابة الصحيحة."
        return ConditionResult(
    ...
    def _handle_answer(
        ...
        condition_id = config.get("id", "multiple_choice")
        try:
            picked = int(str(answer).strip())
        except (TypeError, ValueError):
            picked = -1
        correct = picked == config.get("correct_index")
        ...
        if correct:
            return ConditionResult(
                passed=True, reason="correct_choice",
                message="✅ إجابة صحيحة!",
                condition_id=condition_id,
            )
        attempts = _filter_attempts(
        ...
        return ConditionResult(
            passed=False, reason="wrong_choice",
            requires_interaction=True,
